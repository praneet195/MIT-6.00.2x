...
class RectangularRoom(object):
    ...
    def cleanTileAtPosition(self, pos):
        ...
        pos: a Position
        """
        rex=float(math.floor(pos.x))
        rey=float(math.floor(pos.y))
        repos=Position(rex,rey)
        for i in self.clean.keys():
            if i.x==repos.x and i.y==repos.y:
               return repos
        self.clean[repos]=1
        return repos
    ...
    def isTileCleaned(self, m, n):
        ...
        """
        repos=Position(float(math.floor(m)),float(math.floor(n)))
        for i in self.clean.keys():
            if i.x==repos.x and i.y==repos.y:
                return True
        return False
...
# room = RectangularRoom(1, 3)
# print(room.getNumCleanedTiles())
# print(room.cleanTileAtPosition(Position(0.6, 0.3)))
# print(room.isPositionInRoom(Position(0.6, 0.3)))
# print(room.isTileCleaned(0.6, 0.3))
# # === Problem 3
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for i in range(num_trials):
        ...
        while(1.0*room.getNumCleanedTiles()/room.getNumTiles() <=min_coverage):
            ...
            steps+=1
        triallist.append(steps)
    return (float(sum(triallist)/len(triallist)))
